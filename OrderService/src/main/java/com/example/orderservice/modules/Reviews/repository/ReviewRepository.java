...
//    Page<Reviews> findReviewByProductId(Long productId, Pageable pageable);
   ...
   @Query("SELECT r FROM Reviews r WHERE r.productId = :productId AND r.deletedAt IS NULL")
    Page<Reviews> findReviewByProductId(Long productId, Pageable pageable);
...
//    public List<ProductRatingDto> findProductByRating(Integer rating);
    ...
    // Tìm kiếm các sản phẩm có trung bình trên ngưỡng rating (làm tròn) tự chọn (tham số đầu vào là rating, rating có thể là 1,2,3,4,5) theo entity mới
    @Query("""
        SELECT new com.example.orderservice.modules.Reviews.dto.output.ProductRatingRecord(
                                                            r.productId,
                                                            AVG(r.rating)
                                                          )
        FROM Reviews r
        ...
        HAVING AVG(r.rating) >= :rating
    """)
    public List<ProductRatingRecord> findProductByRating(Integer rating);
